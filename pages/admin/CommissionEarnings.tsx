    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <div className="flex space-x-2 p-1 bg-primary-light rounded-lg">
                    {(['24h', '7d', '30d'] as TimeFilter[]).map(filter => (
                        <button
                            key={filter}
                            onClick={() => setTimeFilter(filter)}
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${timeFilter === filter ? 'bg-accent text-primary' : 'text-gray-300 hover:bg-primary'}`}
                        >
                 ...
                 <Card className="text-center p-4">
                    <h3 className="text-sm text-gray-400">Total Commission ({timeFilter.replace('h', ' hours').replace('d', ' days')})</h3>
            ...
            <Card>
                ...
                <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={chartData}>
                        ...
                        <YAxis stroke="#a8a29e" fontSize={12} tickFormatter={(value) => `$${value}`} />
                        <Tooltip contentStyle={{ backgroundColor: '#041401', border: '1px solid #294A21' }} formatter={(value: number) => [value.toLocaleString('en-US', { style: 'currency', currency: 'USD' }), 'Commission']} />
                    ...
                    <table className="w-full text-sm text-left text-gray-400">
                        ...
                        <tbody>
                            {filteredCommissions.map(c => (
                                <tr key={c.id} className="bg-primary-light border-b border-primary">
                                    <td className="px-6 py-4">{c.date}</td>
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-white">{c.user}</div>
                            ...
                            {filteredCommissions.length === 0 && (
                                <tr>
                                    <td colSpan={5} className="text-center py-8 text-gray-400">No commissions found for this period.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
