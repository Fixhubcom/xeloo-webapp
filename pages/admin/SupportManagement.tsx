            ...
            <div className="lg:col-span-1">
                <Card className="h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto -mr-4 pr-4">
                        {mockTickets.map(ticket => (
                            <div key={ticket.id} className="p-3 bg-primary rounded-md mb-3 cursor-pointer hover:bg-primary-light border-l-4 border-accent">
                                <div className="flex justify-between items-center">
                                    <h3 className="font-semibold text-white">{ticket.subject}</h3>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${ticket.status === 'Open' ? 'bg-green-500/20 text-green-300' : 'bg-gray-500/20 text-gray-300'}`}>{ticket.status}</span>
                                </div>
             ...
             <div className="lg:col-span-2">
                <Card className="h-full flex flex-col">
                    ...
                    <div className="flex-1 bg-primary p-4 rounded-lg overflow-y-auto mb-4 space-y-4">
                        {mockConversation.map((msg, index) => (
                            <div key={index} className={`flex ${msg.from === 'user' ? 'justify-start' : 'justify-end'}`}>
                                <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl ${msg.from === 'user' ? 'bg-gray-medium text-white' : 'bg-accent text-primary'}`}>
                                    <p>{msg.text}</p>
                                </div>
                            </div>
                        ))}
                    ...
                    {/* AI Assistant */}
                     <div className="border border-gray-medium rounded-lg p-4 mb-4">
                        <div className="flex justify-between items-center">
                            <h3 className="text-lg font-semibold text-accent flex items-center"><SparklesIcon className="mr-2"/> Gemini AI Assistant</h3>
                            {!isSuggesting && !suggestion && <button onClick={handleGetSuggestion} className="bg-purple-600 text-white font-bold py-1 px-3 rounded text-sm hover:bg-purple-700">Suggest Reply</button>}
                        </div>
                        {isSuggesting && <div className="flex items-center text-gray-light mt-2"><Spinner className="mr-2"/>Generating...</div>}
                        {suggestion && (
                            <div className="mt-2 bg-primary p-3 rounded-md">
                                <p className="text-gray-light text-sm italic whitespace-pre-wrap">{suggestion}</p>
                    ...
                    <div className="flex space-x-3">
                        <textarea value={reply} onChange={e => setReply(e.target.value)} placeholder="Type your reply..." rows={2} className="flex-1 bg-gray-dark p-2 rounded border border-gray-medium focus:ring-accent focus:border-accent"></textarea>
                        <button className="bg-accent text-primary font-bold px-6 rounded hover:bg-yellow-400 self-end">Send</button>
                    </div>
                </Card>
            </div>
        </div>
